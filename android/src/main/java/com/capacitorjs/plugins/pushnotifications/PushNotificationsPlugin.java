...
public class PushNotificationsPlugin extends Plugin {
    ...
    public static JSObject lastCallAction = null;
    ...
    public static final ChatRoutingInfo chatRoutingInfo = new ChatRoutingInfo();
    ...
    public void load() {
        ...
        if (lastCallAction != null) {
          fireCallActionEvent(lastCallAction);
          lastCallAction = null;
        }
    ...
    protected void handleOnNewIntent(Intent data) {
        ...
        if (bundle != null && bundle.containsKey("voipAction")) {
          JSObject callAction = new JSObject();
          callAction.put("actionId", bundle.getString("voipAction"));
          callAction.put("callId", bundle.getString("callId"));
          fireCallActionEvent(callAction);
        }
    }
    ...
    @PluginMethod
    public void setActiveChatId(PluginCall call) {
      try {
        String activeChatId = call.getString("activeChatId");
        String activeChatType = call.getString("chatType");

        if (activeChatType == null) {
          call.reject("chatType is null");
          return;
        }

        if (activeChatId == null) {
          call.reject("activeChatId is null");
          return;
        }

        if (activeChatType.equals("one-to-one")) {
          chatRoutingInfo.setActiveDirectChatId(activeChatId);
        } else {
          chatRoutingInfo.setActiveGroupChatId(activeChatId);
        }
    ...
    @PluginMethod
    public void clearActiveChatId(PluginCall call) {
      try {
        chatRoutingInfo.setActiveGroupChatId(null);
        chatRoutingInfo.setActiveDirectChatId(null);
    ...
    public void fireCallActionEvent(JSObject callData) {
      notifyListeners("pushNotificationActionPerformed", callData, true);
    }
}
